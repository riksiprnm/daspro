...
void menu();
void initData(string nama, int *no_pesanan, int *jmlHydro, int *jmlLisinopril, int *jmlAntibiotik, int *jmlEpogen, bool *sudahPesan, 
...
void tampilPembayaran(string nama, int no_pesanan, int jmlHydro, int jmlLisinopril, int jmlAntibiotik, int jmlEpogen, 
					  float totalBayar, float totalHarga, float temp, bool member, float diskonMember);
...
int main(int argc, char *argv[]) {
	...
	int no_pesanan, jmlHydrocodone, jmlLisinopril, jmlAntibiotik, jmlEpogen, jumlah;
	double totalHarga;
	float totalBayar, temp;
	int uangPelanggan;
	float uangkembalian;
	float diskonMember;
	initData(nama,&no_pesanan,&jmlHydrocodone,&jmlLisinopril,&jmlAntibiotik,&jmlEpogen,&sudahPesan, 
	...
	do{
		...
		case '2':
			if(cekLog && !sudahPesan){
				...
				do{
					printf("\n\tMasukkan nomer pesanan : "); scanf("%d", &no_pesanan);
					if(no_pesanan%2==0 || (no_pesanan<1 || no_pesanan>30)){
						printf("\n\t\t[!] Nomer pesanan harus ganjil dan diantara 1-30)\n");
					}
				}while(no_pesanan%2==0 || (no_pesanan<1 || no_pesanan>30));
				...
				do{
					do{
						...
						case '1':
							do{
								printf("\n\tJumlah Hydrocodone yang dibeli : "); scanf("%d", &jumlah);
								if(jumlah<1){
						...
						case '2':
							do{
								printf("\n\tJumlah Chocolate Lisinopril yang dibeli : "); scanf("%d", &jumlah);
								if(jumlah<1){
									printf("\n\t\tJumlah yang dibeli tidak boleh kurang dari 1\n");
								}
							}while(jumlah<1);
						...
						case '3':
							do{
								printf("\n\tJumlah Antibiotik yang dibeli : "); scanf("%d", &jumlah);
								if(jumlah<1){
									printf("\n\t\tJumlah yang dibeli tidak boleh kurang dari 1\n");
								}
							}while(jumlah<1);
						...
						case '4':
							do{
								printf("\n\tJumlah Epogen yang dibeli : "); scanf("%d", &jumlah);
								if(jumlah<1){
									printf("\n\t\tJumlah yang dibeli tidak boleh kurang dari 1\n");
								}
							}while(jumlah<1);
		...
		case '3':
			if(sudahPesan && cekLog){
				...
				tampilPembayaran(nama,no_pesanan,jmlHydrocodone,jmlLisinopril,jmlAntibiotik,jmlEpogen,totalBayar, totalHarga,temp, member, diskonMember);
		...
		case '4':
			if(sudahPesan && cekLog){
				...
				if(strcmpi(jawab,"y")==0){
					initData(nama,&no_pesanan,&jmlHydrocodone,&jmlLisinopril,&jmlAntibiotik,&jmlEpogen,&sudahPesan, &totalHarga, &totalBayar, &uangPelanggan, &uangkembalian, &member, &diskonMember);
...
void tampilPembayaran(string nama, int no_pesanan, int jmlHydro, int jmlLisinopril, int jmlAntibiotik, int jmlEpogen, float totalBayar, float totarHarga, float temp, bool member, float diskonMember){
	int temp2;
	printf("\n\t\t--=== Menu Pembayaran ===--\n");
	printf("\n\tNama pemesan \t\t\t: %s",nama);
	printf("\n\tNomer pesanan \t\t\t: %d", no_pesanan);
	printf("\n\n\tJumlah Hydrocodone \t\t: %d", jmlHydro);
	printf("\n\tJumlah Lisinopril \t\t: %d", jmlLisinopril);
	printf("\n\tJumlah Antibiotik Amoxcillin \t: %d", jmlAntibiotik);
	printf("\n\tJumlah Epogen \t\t\t: %d", jmlEpogen);
